...
import { getClients, getusers, registerUser,updateUser } from "../../redux/User/userSlice";
...
function CreateUserModal({ visible, onCancel }) {
  ...
  const validateForm = () => {
    ...
    if (!newUser.accountOverview[0].firstName) {
      errors.firstName = "Le prénom est obligatoire.";
    }

    if (!newUser.accountOverview[0].lastName) {
  ...
  const handleOk = () => {
    ...
    if (isFormValid) {
      dispatch(registerUser({"username": newUser.username,"email": newUser.email,"password": newUser.password,})).then(async(response) =>{
     await   dispatch(updateUser({id: response.payload.id, user: {phoneNumber: newUser.phoneNumber,firstName: newUser.accountOverview[0].firstName,lastName: newUser.accountOverview[0].lastName,user_role: "client" }}))
