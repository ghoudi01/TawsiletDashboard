...
const TicketDetails = ({ match }) => {
  ...
  // // Options array with French labels
  // const options = [
  //   { value: "resolved", label: getLabelInFrench("resolved") },
  //   { value: "open", label: getLabelInFrench("open") },
  //   { value: "inProgress", label: getLabelInFrench("inProgress") },
  // ];
      ...
      <DetailsContainer>
        <DetailsLeft>
          ...
          >
            ...
            >
                     <Link to={`/admin/details/${currentTicket?.command?.documentId}`}>
            {" "}
            <span className="date-finished">#{currentTicket?.command?.refNumber}</span>
          </Link>
