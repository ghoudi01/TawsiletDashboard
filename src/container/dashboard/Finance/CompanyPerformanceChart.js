...
const CompanyPerformanceChart = ({ data, isOwner }) => {
  ...
  return (
    <Card 
      title={<><BarChartOutlined /> Company Performance</>} 
      style={{ height: '100%' }}
    >
          ...
          >
            ...
            <Tooltip 
              formatter={(value, name) => [
                ...
                name === 'orders' ? 'Orders' : name.charAt(0).toUpperCase() + name.slice(1)
              ]}
            />
            <Legend />
      ...
      <div style={{ marginTop: 16, textAlign: 'center' }}>
        <small>{isOwner ? 'Comparison across all companies' : 'Your company performance'}</small>
      </div>
    </Card>
  );
};
