...
const ProfitDistributionChart = ({ data, isOwner, commission, companyId }) => {
  ...
  const transformData = () => {
    ...
    if (isOwner) {
      ...
      return [
        { name: 'Your Profit', value: company.details.beneficeNet * (1 - commission/100) },
        { name: 'Commission', value: company.details.beneficeNet * (commission/100) },
      ];
    }
  };
  ...
  return (
    <Card 
      title={<><PieChartOutlined /> Profit Distribution</>} 
      style={{ height: '100%' }}
    >
      ...
      <div style={{ marginTop: 16, textAlign: 'center' }}>
        <small>{isOwner ? 'Distribution across companies' : 'Your profit vs platform commission'}</small>
      </div>
    </Card>
  );
};
